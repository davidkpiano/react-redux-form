    ...
    describe('onLoad prop', () => {
      ...
      it('should call the onLoad function', () => {
        ...
        if (testKey === 'standard') {
          ...
          assert.equal(handleLoad.args[0][1].get('initialValue'), 'bar');
        }
  ...
  describe('Extended Control components', () => {
    ...
    describe('ignoring events with ignore prop', () => {
      ...
      it('ignores the events specified in the ignore prop', () => {
        assert.isFalse(toJS(store.getState().testForm).foo.focus);
        ...
        assert.isFalse(toJS(store.getState().testForm).foo.touched,
    ...
    describe('validateOn != updateOn', () => {
      ...
      it('should initially be valid', () => {
        assert.isTrue(toJS(radioStore.getState().testForm).foo.valid);
      });
      ...
      it('should still be valid after focusing', () => {
        ...
        assert.isTrue(toJS(radioStore.getState().testForm).foo.valid);
        assert.isTrue(toJS(radioStore.getState().testForm).foo.focus);
      });
    });
