...
var controlPropsMap = exports.controlPropsMap = {
  'text': function text(props, eventProps) {
    return _extends({
      name: props.model,
      defaultValue: props.modelValue
    }, eventProps);
  },
  'textarea': function textarea(props, eventProps) {
    return controlPropsMap.text(props, eventProps);
  },
  'checkbox': function checkbox(props, eventProps) {
    return _extends({
      name: props.model,
  ...
  'radio': function radio(props, eventProps) {
    return _extends({
      name: props.model,
  ...
  'select': function select(props, eventProps) {
    return _extends({
      name: props.model,
      value: props.modelValue
    }, eventProps);
  },
  'default': function _default(props, eventProps) {
    return controlPropsMap.text(props, eventProps);
  }
};
...
function getControlType(control, options) {
  var controlTypesMap = options.controlTypesMap;
  var controlPropsMap = options.controlPropsMap;

  var controlType = control.type.name || control.type;
  var mappedControlType = controlTypesMap[controlType] || controlType;

  var finalControlType = mappedControlType === 'input' ? control.props.type : mappedControlType;

  if (!controlPropsMap[finalControlType]) {
    finalControlType = mappedControlType === 'input' ? 'text' : null;
  }
...
function createFieldProps(control, props, options) {
  ...
  return _extends({}, controlPropsMap[controlType]({
    model: model,
  ...
  if (!controlProps) {
    return _react2.default.cloneElement(control, {
      children: _react2.default.Children.map(control.props.children, function (child) {
        return createField(child, _extends({}, props, child.props), options);
      })
    });
  }
...
function createFieldClass() {
  var _controlTypesMap = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];

  var _controlPropsMap = arguments.length <= 1 || arguments[1] === undefined ? controlPropsMap : arguments[1];
