...
function getControlType(control, options) {
  ...
  mappedControlType = mappedControlType === 'input' ? control.props.type : mappedControlType;

  if (!controlPropsMap[mappedControlType]) {
    mappedControlType = control.type === 'input' ? 'text' : null;
  }
...
function createFieldProps(control, props, options) {
  ...
  return _extends({}, controlPropsMap[controlType]({
    ...
    value: value
  }, control), sequenceEventActions(control, props, options));
}
