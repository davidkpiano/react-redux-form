...
function getControlDisplayName(control) {
  try {
    return control.constructor.displayName || control.type.displayName || control.type.name || control.type + '';
  } catch (e) {
    return undefined;
  }
}
...
function getControlType(control, options) {
  ...
  var controlType = getControlDisplayName(control);
...
function createFieldClass() {
  ...
  var customControlTypesMap = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
