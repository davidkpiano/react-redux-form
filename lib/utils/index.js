...
function getEventValue(event) {
  return event.target ? event.target.multiple ? [].concat(_toConsumableArray(event.target.selectedOptions)).map(function (option) {
    return option.value;
  }) : event.target.value : event.nativeEvent ? event.nativeEvent.text : undefined;
}
